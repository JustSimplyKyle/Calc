...
double big_calc(vector<double> numbers, vector<char> math_calc)
{
    ...
    for (int i = 0; i < math_calc.size(); i++) // calculate power first
    {
        if (math_calc[i] == '^' || math_calc[i] == '!')
        {
            ...
            i--; // to comphensate for the erase movement
        }
    }
...
int main()
{
    ...
    for (int k = 0; k < para_size; k++)
    {
            ...
            if (math_calc[i] == '(')
            {
                ...
                for (int j = 0; j <= math_calc.size(); j++)
                {
                    if (math_calc[j] == '(')
                    {
                        ...
                        for (int w = 0; w <= j; w++) //find how many parathenis are in the current iteration
                        {
                            if (math_calc[w] == '(')
                            {
                                now_para_size++;
        ...
        // start reseting variables
        in_iter = math_calc.begin();
        out_iter = math_calc.begin();
        in = 0;
        out = 0;
        temp_ch.clear();
        temp_numbers.clear();
